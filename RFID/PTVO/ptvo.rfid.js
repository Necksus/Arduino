...
// Override default PTVO library converter to prevent future breaking changes
fz.ptvo_switch_uart_legacy = {
...
const localFromZigbee = [fz.ptvo_switch_uart_legacy, /*fz.ptvo_on_off,*/ fz.ptvo_multistate_action, fz.on_off];
...
const device = {
    ...
    toZigbee: [tz.ptvo_switch_trigger, tz.ptvo_switch_uart, tz.on_off],
    exposes: [
      exposes.text('action', ea.STATE_SET).withDescription('RFID tag scanned'),
//      e.contact().withEndpoint('l2'),
      e.switch().withEndpoint('Beep').withDescription('Turn on/off the beeper'),
//      e.contact().withEndpoint('l4'),
    ...
    endpoint: (device) => {
        return {
            l1: 1,
            action: 1,
//            l2: 2,    // This endpoint is not used
            Beep: 3,    // Turn on/off the beeper
//            l4: 4,    // This endpoint is not used
        };
    },
